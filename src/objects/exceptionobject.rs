...
fn nameexc_repr(selfv: Object<'_>) -> MethodType<'_> {
    ...
    MethodValue::Some(stringobject::string_from(selfv.vm.clone(), format!("NameExc: \"{}\"", repr.unwrap())))
}
...
#[allow(dead_code)]
pub fn overflowexc_from_obj<'a>(vm: Arc<VM<'a>>, obj: Object<'a>, start: Position, end: Position) -> Object<'a> {
...
pub fn overflowexc_from_str<'a>(vm: Arc<VM<'a>>, raw: &str, start: Position, end: Position) -> Object<'a> {
    let tp = create_object_from_type(vm.get_type("OverflowExc"));
    unsafe {
        let refr = Arc::into_raw(tp.clone()) as *mut RawObject<'a>;
        (*refr).internals = ObjectInternals::Exc(ExcData {obj: stringobject::string_from(vm.clone(), raw.to_string()), start, end});
...
fn overflowexc_new<'a>(_selfv: Object<'a>, _args: Object<'a>, _kwargs: Object<'a>) -> MethodType<'a> {
    unimplemented!();
}
fn overflowexc_repr(selfv: Object<'_>) -> MethodType<'_> {
    ...
    MethodValue::Some(stringobject::string_from(selfv.vm.clone(), format!("OverflowExc: \"{}\"", repr.unwrap())))
}
