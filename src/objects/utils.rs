...
pub fn object_repr_safe(object: &Object) -> Result<String, String> {
    let repr = object.clone().repr();
    if repr.is_not_implemented() {
        return Err(String::from("__repr__ is not implemented."));
    }
    
    let reprv = repr.unwrap().get_basic_repr();
    
    if reprv.is_not_implemented() {
        return Err(String::from("__repr__ returned non-string."));
    }

    return Ok(reprv.unwrap());
}